...
<div class="Main-page">
	  <div class="container">
		 ...
		 <h1 style="text-align: center; font-family:'Do Hyeon', sans-serif; font-size: 44px;">이상현상 발생 현황 조회</h1>
		  ...
		  <button class="btn-3" style="float: right; ">logout</button>
						 		...
						 		<form class="check" method="POST" action="check_page.php?page=1">
		  <button class="btn-4" style="float: right; ">records</button>
...
$mysql_query = "set names utf8";
	$data = "select emp_id from hospital_emer_check";
...
right join hospital_emer_check on emp_info.emp_id=hospital_emer_check.emp_id order by seq desc limit ".$s_point.",".$list;
$result = mysqli_query($connect,$query) or die(mysqli_error($connect));
	...
	while($i<$cnt)
	{
		...
		echo '<td><form action="delete_row.php" method="post">';#########################삭제 버튼 id = $seq[$j+1]
		echo '<input type="hidden" name="seq" value=',$ary[$j],'>';   ## "" 안에 시퀀스 값 넣을 것
		...
		echo '<tr>';
		echo '<td colspan = "8" align="center"><a href="main.php?page=',$s_page-1,'">이전</a>';
	for($p=$s_page; $p<=$e_page; $p++) {

		echo '<a href="main.php?page=',$p,'">',$p,'</a>';
		echo "&nbsp;";
		}
		
	
		echo '<a href="main.php?page=',$e_page+1,'">다음</a>';
		echo "</td>";
				echo "</tr>";
...
else
{
		...
		if ($_SESSION['count'] != $temp)  // emp_emer 테이블에 새 행이 들어왔을 때 실행
	{
		...
		$Emp_name_rows = mysqli_fetch_array($Emp_name);
			...
			echo "<script language=javascript>alert('{$Emp_name_rows[0]}님 이상 발생!! 관리자에게 연락해 주시기 바랍니다. ');</script>";
